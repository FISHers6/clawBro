<div align="center">
  ...
  <h1>🦀 clawBro：Coding CLI Agent を OpenClaw のように、チャットとチーム協業の中で本当に働かせる</h1>
  <p>
    <strong>OpenClaw の発想を土台に、Claude Code、Codex、Qwen、Qoder、Gemini などの coding agent CLI を協調動作させ、Lark、DingTalk、チームワークフローへ接続します。</strong>
  </p>
  <p>
  ...
  <p>
    ...
    <a href="#-coding-agent-統合">Coding Agent 統合</a> ·
...
`clawBro` は Rust で書かれた AI 協業システムです。単なるチャットボットのラッパーではなく、複数の coding agent CLI を実際に一緒に働かせることを目的としています。

OpenClaw の方向性を受け継ぎつつ、より実務的な協業に寄せています。Claude Code、Codex、Qwen、Qoder、Gemini などを同じワークフローに組み込み、DM、グループチャット、Lark、DingTalk、WebSocket、Team モードで連携させられます。

## 📢 プロジェクト状況

- **[03-19]** 複数の AI coding CLI を 1 つの協業フローにまとめ、ツールごとに別々の運用をしなくてよくなりました。
- **[03-19]** 現在もっとも安定している形は、Lead が外向きの応答を担い、specialist が裏側で実行し、Lead が milestone をまとめる形です。
- **[03-19]** Lark、DingTalk Stream Mode、DingTalk Custom Robot Webhook、WebSocket に接続でき、ローカル利用からグループ利用まで段階的に広げられます。
- **[03-19]** 複数 IM に同時常駐できるため、1 つの `clawbro` ランタイムを Lark・DingTalk・チーム会話へ常時つないだまま運用できます。
...
🤖 **統合 Coding Agent**：Claude Code、Codex、Qwen、Qoder、Gemini などを同じ仕組みの中で動かせます。
...
- [定時タスク](#-定時タスク)
- [チームモード](#-チームモード)
- [Coding Agent 統合](#-coding-agent-統合)
- [チャットチャンネル](#-チャットチャンネル)
- [設定と運用](#️-設定と運用)
- [プロジェクト構成](#️-プロジェクト構成)
- [ドキュメント](#-ドキュメント)
...
> [!TIP]
> 最初は `WebSocket + ClawBro Native` の最小構成がおすすめです。
...
```bash
# GitHub Release: 展開してそのまま実行
./clawbro --version

# npm: インストール時に対応バイナリを取得
npm install -g clawbro
clawbro --version
```
...
| **Coding CLI bridge** | 外部 coding CLI の互換層 | 複数の coding agent を同じ使い方に統一 |
...
- OpenClaw と coding agent CLI の使い方をまとめたい設計者
