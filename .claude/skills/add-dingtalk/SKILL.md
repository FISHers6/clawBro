...
引导你完成钉钉 Channel 的接入配置。完成后，ClawBro Gateway 可以：
- 接收钉钉群消息（机器人 @mention）
- 接收钉钉单聊消息
- 回复群消息和单聊消息
...
```bash
grep -q '\[channels.dingtalk\]' ~/.clawbro/config.toml 2>/dev/null && echo "⚠ 已有 DingTalk 配置，本次将更新" || echo "✓ 将新增 DingTalk 配置"
```
...
```bash
grep -A 3 '\[\[agent_roster\]\]' ~/.clawbro/config.toml 2>/dev/null || echo "（无 agent_roster 配置，将使用默认 backend）"
```
...
curl -s -X POST "https://oapi.dingtalk.com/robot/send?access_token=$ACCESS_TOKEN" \
  ...
  -d '{"msgtype":"text","text":{"content":"ClawBro Gateway 连通性测试"}}'
```
