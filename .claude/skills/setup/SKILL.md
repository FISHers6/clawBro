---
name: setup
description: 从零引导配置 clawbro-gateway，包括 API Key、默认 Agent、运行模式（Solo/Multi-agent/Team）、Channel（Lark/DingTalk）。无需 fork 或重新编译。
---
...
```bash
which clawbro-gateway || ls ./target/release/clawbro-gateway 2>/dev/null || ls ./target/debug/clawbro-gateway 2>/dev/null
which clawbro-rust-agent || ls ./target/release/clawbro-rust-agent 2>/dev/null || ls ./target/debug/clawbro-rust-agent 2>/dev/null
```
...
```bash
cp target/debug/clawbro-gateway ~/.local/bin/
cp target/debug/clawbro-rust-agent ~/.local/bin/
...
```bash
[ -f ~/.clawbro/config.toml ] && echo "⚠ 已存在 config.toml，本次将覆盖" || echo "✓ 干净环境，将创建新配置"
```
...
export CLAWBRO_MODEL=<用户填写的model>
EOF
```
...
if [ -n "$SHELL_PROFILE" ]; then
  grep -q "clawbro/.env" "$SHELL_PROFILE" 2>/dev/null \
    && echo "✓ 已存在，跳过" \
    || echo 'source ~/.clawbro/.env' >> "$SHELL_PROFILE" && echo "✓ 已写入 $SHELL_PROFILE"
fi
```
...
[session]
dir = "/Users/<username>/.clawbro/sessions"

[memory]
shared_dir = "/Users/<username>/.clawbro/shared"
distill_every_n = 20
distiller_binary = "clawbro-rust-agent"

[skills]
dir = "/Users/<username>/.clawbro/skills"
```
...
```bash
# 备份现有配置（如果存在）
[ -f ~/.clawbro/config.toml ] && cp ~/.clawbro/config.toml ~/.clawbro/config.toml.bak.$(date +%Y%m%d%H%M%S) && echo "✓ 旧配置已备份"

# 写入新配置
cat > ~/.clawbro/config.toml << 'TOMLEOF'
<根据 Phase 3 生成的完整 TOML 内容>
TOMLEOF
echo "✓ 配置已写入 ~/.clawbro/config.toml"
```
...
```bash
clawbro-gateway --validate-only 2>&1 | head -20
```
...
PORT=$(cat ~/.clawbro/gateway.port 2>/dev/null || echo "8080")
TOKEN=$(grep 'ws_token' ~/.clawbro/config.toml 2>/dev/null | sed 's/.*= *"//' | sed 's/".*//')
...
[ -n "$BACKUP" ] && cp "$BACKUP" ~/.clawbro/config.toml && echo "✓ 已恢复 $BACKUP" || echo "无备份可恢复"
```
