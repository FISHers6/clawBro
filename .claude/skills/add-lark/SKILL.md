...
引导你完成飞书 Channel 的接入配置。完成后，QuickAI Gateway 可以：
- 接收飞书群消息 / 单聊消息
- 回复飞书消息
- 在群里被 @mention 触发
...
```bash
grep -q '\[channels.lark\]' ~/.quickai/config.toml 2>/dev/null && echo "⚠ 已有 Lark 配置，本次将更新" || echo "✓ 将新增 Lark 配置"
```
...
```bash
# 查看已有 agent_roster
grep -A 3 '\[\[agent_roster\]\]' ~/.quickai/config.toml 2>/dev/null || echo "（无 agent_roster 配置，将使用 [agent] backend_id）"
```
...
PORT=$(cat ~/.quickai/gateway.port 2>/dev/null || grep 'port' ~/.quickai/config.toml | head -1 | grep -o '[0-9]*' || echo "8080")
...
bot_name           = "QuickAI"                           # 机器人名称（群消息 @mention 识别）
...
- 检查 Gateway 日志：`tail -f ~/.quickai/gateway.log`
