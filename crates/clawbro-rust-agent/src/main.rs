//! clawbro-rust-agent — ACP Server binary
//! Communicates over stdio using the Agent Client Protocol.
//!
//! Usage:
//!   ANTHROPIC_API_KEY=sk-... clawbro-rust-agent
//!   RUST_LOG=info ANTHROPIC_API_KEY=sk-... clawbro-rust-agent
//!
...
use clawbro_rust_agent::{agent, config, native_runtime};
...
async fn main() -> anyhow::Result<()> {
    ...
    if agent_config.is_some() {
        tracing::info!("Starting clawbro-rust-agent with rig-core LLM engine");
    } else {
        tracing::info!("Starting clawbro-rust-agent in echo stub mode (no API key set)");
    }
