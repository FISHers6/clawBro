//! ACP integration tests for quickai-rust-agent.
//!
...
fn spawn_stub() -> tokio::process::Child {
    Command::new(env!("CARGO_BIN_EXE_quickai-rust-agent"))
        ...
        .stderr(Stdio::null())
        .spawn()
        .expect("failed to spawn quickai-rust-agent")
}
...
async fn test_full_rig_prompt() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_quickai-rust-agent"))
        .env("ANTHROPIC_API_KEY", &api_key)
        .env("QUICKAI_MODEL", "claude-haiku-4-5-20251001")
        .env("QUICKAI_SYSTEM_PROMPT", "Reply in one word only.")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
