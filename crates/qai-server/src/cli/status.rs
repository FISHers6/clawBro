...
pub async fn run() -> Result<()> {
    ...
    println!("{}", style("QuickAI Gateway — Status").bold().cyan());
    println!("{}", "─".repeat(40));

    if !cfg_path.exists() {
        println!("{} config.toml not found — run: quickai setup", style("⚠").yellow());
        return Ok(());
    }
    ...
    println!(
        ...
        if has_key {
            ...
            style("not set (source ~/.quickai/.env)").yellow().to_string()
        }
    );
