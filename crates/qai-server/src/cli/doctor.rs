...
pub async fn run() -> Result<()> {
    ...
    for bin in ["quickai-gateway", "quickai-rust-agent"] {
        match which::which(bin) {
            ...
            Err(_) => {
                println!("  {} {} not found — check PATH or rebuild", style("✗").red(), bin);
                issues += 1;
            }
        }
    }
    ...
    if cfg_path.exists() {
        ...
        match toml::from_str::<toml::Value>(&content) {
            Ok(_)  => println!("  {} ~/.quickai/config.toml (valid TOML)", style("✓").green()),
            Err(e) => {
        ...
        println!("  {} config.toml missing — run: quickai setup", style("✗").red());
        issues += 1;
    }
    ...
    if env_path.exists() {
        println!("  {} ~/.quickai/.env exists", style("✓").green());
    } else {
        println!("  {} ~/.quickai/.env missing", style("–").yellow());
    }
    for var in ["ANTHROPIC_API_KEY", "OPENAI_API_KEY"] {
        match std::env::var(var) {
            Ok(v) if !v.is_empty() => println!("  {} {} set (current shell)", style("✓").green(), var),
            _ => println!("  {} {} not set (run: source ~/.quickai/.env)", style("–").yellow(), var),
        }
    }
    ...
    for sub in ["sessions", "shared", "skills"] {
        ...
        if p.exists() {
            ...
            println!("  {} ~/.quickai/{} missing (mkdir -p ~/.quickai/{})", style("✗").red(), sub, sub);
            issues += 1;
        }
    }
    ...
    if port_file.exists() {
        ...
        println!("  {} Not running (gateway.port missing)", style("–").yellow());
    }
    ...
    if issues == 0 {
        ...
        println!("{}", style(format!("{} issue(s) found — see above", issues)).bold().yellow());
    }
    Ok(())
}
