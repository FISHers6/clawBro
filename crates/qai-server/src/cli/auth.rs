...
fn cmd_set(provider: &str, key: &str) -> Result<()> {
    ...
    save_env_map(&map)?;
    println!("{} {} updated (~/.quickai/.env)", style("✓").green(), var);
    println!("  Reload: source ~/.quickai/.env");
    Ok(())
}
...
fn cmd_list() -> Result<()> {
    ...
    if !found {
        println!(
            "  {} No API keys found (~/.quickai/.env is missing or empty)",
            style("–").yellow()
        );
        println!("  Run: quickai auth set anthropic <key>");
    }
    Ok(())
}
