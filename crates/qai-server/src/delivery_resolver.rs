...
use qai_agent::TurnDeliverySource;
use qai_protocol::{OutboundMsg, SessionKey};
use std::sync::Arc;

#[derive(Clone)]
pub struct ResolvedDelivery {
    pub sender: Arc<dyn qai_channels::Channel>,
...
impl ResolvedDelivery {
    pub fn outbound_text(&self, text: impl AsRef<str>) -> OutboundMsg {
        OutboundMsg {
            ...
            content: qai_protocol::MsgContent::text(text.as_ref()),
