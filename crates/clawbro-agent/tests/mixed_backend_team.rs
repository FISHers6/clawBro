...
use clawbro_protocol::{render_scope_storage_key, InboundMsg, MsgContent, MsgSource, SessionKey};
use clawbro_runtime::{
...
struct TeamHarness {
    ...
    _event_rx: broadcast::Receiver<clawbro_protocol::AgentEvent>,
}
...
async fn build_team_harness(
    ...
    let (team_notify_tx, mut team_notify_rx) =
        mpsc::channel::<clawbro_agent::team::completion_routing::TeamNotifyRequest>(32);
...
fn has_all_files(capture: &clawbro_runtime::CapturedTurn, names: &[&str]) -> bool {
    names.iter().all(|name| {
        capture
            .session
            .context
...
fn has_scoped_memory_file(capture: &clawbro_runtime::CapturedTurn, channel: &str) -> bool {
...
async fn mixed_backend_smoke_acp_lead_native_specialist_submit_and_accept() {
    ...
    assert!(leader_captures.iter().any(|capture| {
        capture.session.role == clawbro_runtime::RuntimeRole::Leader
            && has_all_files(
                capture,
                &[
                    "SOUL.md",
    ...
    assert_eq!(
        ...
        clawbro_agent::team::orchestrator::TeamState::Done
    );
...
async fn mixed_backend_smoke_native_lead_acp_specialist_submit_and_accept() {
    ...
    let specialist_adapter = Arc::new(ScriptedAdapter::new(
        ...
        |_spec, session| {
            assert_eq!(session.role, clawbro_runtime::RuntimeRole::Specialist);
            Ok(ScriptedTurn {
    ...
    assert!(leader_captures
        .iter()
        .any(|capture| capture.session.role == clawbro_runtime::RuntimeRole::Leader));
    assert!(leader_captures.iter().any(|capture| {
        has_all_files(
            capture,
            &[
    ...
    assert!(specialist_captures
        .iter()
        .all(|capture| capture.session.role == clawbro_runtime::RuntimeRole::Specialist));
    assert!(specialist_captures
        .iter()
