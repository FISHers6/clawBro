...
impl BackendAdapter for OpenClawBackendAdapter {
    ...
    async fn run_turn(
        ...
        let specialist_bridge = helper.as_ref().and_then(|_| {
            (session.role == crate::contract::RuntimeRole::Specialist).then(|| {
        ...
        let lead_bridge = helper.as_ref().and_then(|_| {
            (session.role == crate::contract::RuntimeRole::Leader).then(|| {
        ...
        loop {
                ...
                .await?
            {
                ...
                Some(GatewayInbound::FinalText(text)) => {
                    sink.emit(crate::contract::RuntimeEvent::TurnComplete {
...
fn resolve_team_helper_config(
    ...
    let path = Path::new(command);
    if !path.is_absolute() {
        anyhow::bail!("OpenClaw team helper must use an absolute path: {command}");
    }
...
fn augment_prompt_for_openclaw(
    ...
    let Some(helper) = helper else {
        return crate::contract::render_runtime_prompt(session);
    };
    let role_notes = match session.role {
        crate::contract::RuntimeRole::Leader => [
            format!(
        ...
        .join("\n"),
        crate::contract::RuntimeRole::Specialist => [
            format!(
        ...
        .join("\n"),
        crate::contract::RuntimeRole::Solo => String::new(),
    };
...
mod tests {
    ...
    fn helper_result_is_normalized_into_canonical_tool_callback() {
        let bridge = OpenClawTeamBridge::new(OpenClawRunBinding {
            ...
            task_id: String::new(),
            session_key: qai_protocol::SessionKey::new("specialist", "team-test:openclaw-main"),
        ...
        let event = bridge
            .handle_helper_result(&crate::render_team_helper_success(
                "submit_task_result",
        ...
        assert!(matches!(
            event,
            RuntimeEvent::ToolCallback(crate::contract::TeamCallback::TaskSubmitted {
                ref task_id,
                ref summary,
                ref agent,
        ...
        let err = adapter
            .run_turn(
                ...
                RuntimeSessionSpec {
                    ...
                    session_key: qai_protocol::SessionKey::new("ws", "openclaw:test"),
    ...
    fn openclaw_prompt_includes_helper_commands_for_specialists() {
        ...
        assert!(prompt.contains("/opt/qai-team-cli"));
    ...
    fn openclaw_prompt_includes_helper_commands_for_leaders() {
        ...
        let session = RuntimeSessionSpec {
            ...
            session_key: qai_protocol::SessionKey::new("ws", "group:team"),
