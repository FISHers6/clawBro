...
use crate::bridge::{RuntimeProviderProfile, RuntimeProviderProtocol};
...
impl AgentConfig {
    ...
    /// Model: QUICKAI_MODEL (defaults vary by provider)
    /// System prompt: QUICKAI_SYSTEM_PROMPT
    pub fn from_env() -> Result<Self> {
        let system_prompt = std::env::var("QUICKAI_SYSTEM_PROMPT")
        ...
        if let Ok(key) = std::env::var("ANTHROPIC_API_KEY") {
            return Ok(Self {
                ...
                api_key: key,
                model: std::env::var("QUICKAI_MODEL")
        ...
        if let Ok(key) = std::env::var("OPENAI_API_KEY") {
            ...
            return Ok(Self {
                ...
                api_key: key,
                model: std::env::var("QUICKAI_MODEL").unwrap_or(default_model),
                system_prompt,
            });
        }
...
mod tests {
    ...
    fn test_config_anthropic() {
        ...
        unsafe {
            ...
            std::env::remove_var("QUICKAI_MODEL");
        }
    ...
    fn test_config_openai_base_url() {
        ...
        unsafe {
            ...
            std::env::set_var("QUICKAI_MODEL", "deepseek-chat");
        }
        let config = AgentConfig::from_env().unwrap();
        assert!(matches!(
            config.provider,
