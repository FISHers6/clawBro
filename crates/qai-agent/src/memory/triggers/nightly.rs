...
impl MemoryTrigger for NightlyConsolidationTrigger {
    fn name(&self) -> &str { "nightly_consolidation" }
...
mod tests {
    ...
    async fn test_nightly_consolidation_reads_all_agents_and_writes_shared() {
        ...
        store.overwrite_agent_memory(agent_a.path(), &scope, "agent_a知道：Go + PG").await.unwrap();
        store.overwrite_agent_memory(agent_b.path(), &scope, "agent_b知道：Redis lock").await.unwrap();
