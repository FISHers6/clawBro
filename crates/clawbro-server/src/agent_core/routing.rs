...
use crate::agent_core::team::orchestrator::{TeamOrchestrator, TeamState};
use crate::agent_core::team::session::stable_team_id_for_session_key;
use crate::agent_core::traits::AgentRole;
use crate::protocol::{InboundMsg, MsgSource, SessionKey, TeamTool};
use crate::runtime::contract::{TurnIntent, TurnMode};
...
pub(crate) fn resolve_turn_routing(
    ...
    let task_reminder = if early_is_lead {
        build_lead_task_reminder(inbound, session_team_orch.as_ref())
    } else {
        specialist_task_reminder
    };
    let allowed_team_tools = if early_is_lead {
        build_lead_team_tool_allowlist(inbound)
    } else {
        vec![]
    };
...
fn build_lead_task_reminder(
    inbound: &InboundMsg,
    team_orch: Option<&Arc<TeamOrchestrator>>,
) -> Option<String> {
    if inbound.source == MsgSource::TeamNotify {
        if let Some(reminder) = build_team_notify_resolution_reminder(inbound) {
        ...
        TeamState::Running | TeamState::Done => {
            format!(
                "团队任务执行中。可分配的 Specialist：{specialists_list}\n\n\
                 ...
                 - submitted 待验收这一轮必须先检查结果工件，再调用 accept_task / reopen_task，不能静默结束\n\
...
fn build_team_notify_resolution_reminder(inbound: &InboundMsg) -> Option<String> {
    let text = inbound.content.as_text()?;
    if !text.starts_with("[团队通知]") {
        return None;
    }

    let task_id = extract_team_notify_task_id(text).unwrap_or("当前任务");
    let artifact_path = extract_result_artifact_path(text);

    if text.contains("已提交待验收") {
        let mut reminder = format!(
            "这是任务验收回合。你正在处理 {task_id} 的 submitted 待验收通知。\n\n\
             必须先检查该任务的结果工件和必要上下文，然后在结束前**恰好调用一个**决策工具：\n\
             - accept_task(task_id=\"{task_id}\")\n\
             - reopen_task(task_id=\"{task_id}\", reason=\"...\")\n\n\
             不能静默结束，不能跳过工具调用，也不要在这一轮直接 post_update 最终结果。"
        );
        if let Some(path) = artifact_path {
            reminder.push_str(&format!(
                "\n\n优先检查结果工件：`{path}`。如有必要，再查看同目录下的 `progress.md`、`spec.md`、`plan.md`。"
            ));
        }
        return Some(reminder);
    }

    if text.contains("未调用任何 canonical team tool") {
        return Some(format!(
            "这是 Lead 纠偏回合。{task_id} 的 Specialist 返回了无效完成结果。\n\n\
             你必须决定下一步动作，不能静默结束：\n\
             - 重新 assign_task(task_id=\"{task_id}\", agent=\"...\")，或\n\
             - post_update(...) 告知用户并说明处理方案。"
        ));
    }

    if text.contains("已阻塞") {
        return Some(format!(
            "这是阻塞处理回合。{task_id} 当前已阻塞。\n\n\
             你必须明确处理，不能静默结束：\n\
             - assign_task(task_id=\"{task_id}\", agent=\"...\") 重新分配，或\n\
             - post_update(...) 告知用户当前阻塞情况。"
        ));
    }

    if text.contains("永久失败") {
        return Some(format!(
            "这是失败处理回合。{task_id} 已永久失败。\n\n\
             你必须在这一轮明确处理，不能静默结束；至少需要调用 post_update(...) 向用户说明失败原因和下一步。"
        ));
    }
...
fn build_lead_team_tool_allowlist(inbound: &InboundMsg) -> Vec<TeamTool> {
    let Some(text) = inbound.content.as_text() else {
        return vec![];
    };
    if inbound.source != MsgSource::TeamNotify || !text.starts_with("[团队通知]") {
        return vec![];
    }

    if text.contains("已提交待验收") {
        return vec![TeamTool::AcceptTask, TeamTool::ReopenTask];
    }
    if text.contains("未调用任何 canonical team tool") {
        return vec![TeamTool::AssignTask, TeamTool::ReopenTask];
    }
    if text.contains("已阻塞") {
        return vec![TeamTool::AssignTask, TeamTool::PostUpdate];
    }
    if text.contains("永久失败") {
        return vec![TeamTool::PostUpdate];
    }
    vec![]
}

fn extract_team_notify_task_id(text: &str) -> Option<&str> {
    text.find("任务 ")
        .and_then(|idx| text.get(idx + 3..))
...
mod tests {
    ...
    #[test]
    fn submitted_team_notify_builds_strict_review_reminder() {
        let roster = make_roster();
        let orch = make_orchestrator();
        let orchestrators = DashMap::new();
        orchestrators.insert(
        ...
        let inbound = InboundMsg {
            ...
            content: MsgContent::text(
                "[团队通知] 任务 T001 已提交待验收（执行者：codex）\n\n提交摘要：\n已完成\n\n完整结果工件：tasks/T001/result.md",
            ),
            sender: "gateway".into(),
            channel: "lark".into(),
            timestamp: chrono::Utc::now(),
        ...
        let reminder = decision.task_reminder.expect("lead reminder must exist");
        assert!(reminder.contains("这是任务验收回合"));
        assert!(reminder.contains("accept_task("));
        assert!(reminder.contains("reopen_task("));
        assert!(reminder.contains("T001"));
        assert!(reminder.contains("tasks/T001/result.md"));
        assert!(reminder.contains("不能静默结束"));
    }

    #[test]
    fn submitted_team_notify_allowlist_only_contains_review_actions() {
        let inbound = InboundMsg {
            id: "notify-review-tools".into(),
            session_key: SessionKey::new("lark", "group:route"),
            content: MsgContent::text(
                "[团队通知] 任务 T002 已提交待验收。\n完整结果工件：tasks/T002/result.md",
            ),
            sender: "gateway".into(),
            channel: "lark".into(),
            timestamp: chrono::Utc::now(),
        ...
        assert_eq!(
            build_lead_team_tool_allowlist(&inbound),
            vec![TeamTool::AcceptTask, TeamTool::ReopenTask]
        );
    }

    #[test]
    fn failed_team_notify_allowlist_only_contains_post_update() {
        let inbound = InboundMsg {
            id: "notify-failed-tools".into(),
            session_key: SessionKey::new("lark", "group:route"),
            content: MsgContent::text("[团队通知] 任务 T004 已永久失败。"),
            sender: "gateway".into(),
            channel: "lark".into(),
            timestamp: chrono::Utc::now(),
            thread_ts: None,
