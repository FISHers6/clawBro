...
pub struct NTurnDistillTrigger {
    n: u64,
}
...
impl MemoryTrigger for NTurnDistillTrigger {
    ...
    async fn fire(
        ...
        if let MemoryEvent::TurnCompleted {
            scope, persona_dir, ..
        } = event
        {
...
mod tests {
    ...
    async fn test_distills_and_overwrites_agent_memory() {
        ...
        let mem = store
            .load_agent_memory(persona.path(), &SessionKey::new("dingtalk", "g1"))
            .await
            .unwrap();
        assert!(
            !mem.is_empty(),
            "memory should be written after distillation"
        );
    }
}
