...
use super::completion_routing::{
    PendingRoutingRecord, RoutingDeliveryStatus, TeamNotifyRequest, TeamRoutingEnvelope,
    TeamRoutingEvent,
};
...
pub struct TeamRuntimeSummary {
    pub team_id: String,
    pub state: TeamState,
    pub lead_session_key: Option<crate::protocol::SessionKey>,
    pub lead_agent_name: Option<String>,
    pub latest_leader_update: Option<crate::agent_core::team::session::LeaderUpdateRecord>,
...
pub struct TeamOrchestrator {
    ...
    scope: std::sync::OnceLock<crate::protocol::SessionKey>,
    ...
    lead_session_key: std::sync::Mutex<Option<crate::protocol::SessionKey>>,
...
#[derive(Debug, Clone)]
struct ActiveLeadReviewContext {
    run_id: String,
    task_id: String,
}
...
impl TeamOrchestrator {
    ...
    pub fn bootstrap_workspace_artifacts(&self) -> Result<()> {
        let lead_name = self
            .lead_agent_name
            .get()
            .map(|s| s.as_str())
        ...
        if self.session.read_team_md().is_empty() {
        ...
        self.session.write_agents_md(&agents_md)?;

        if !self.session.dir.join("CONTEXT.md").exists() {
            self.session.write_context_md("")?;
        }

        if !self.session.dir.join("HEARTBEAT.md").exists() {
            self.session.write_heartbeat_md("")?;
        }

        std::fs::create_dir_all(self.session.tasks_dir())?;
    ...
    pub fn set_scope(&self, scope: crate::protocol::SessionKey) {
    ...
    pub fn set_lead_session_key(&self, key: crate::protocol::SessionKey) {
        *self.lead_session_key.lock().unwrap() = Some(key);
        self.flush_pending_routing_events();
    }

    pub fn lead_session_key(&self) -> Option<crate::protocol::SessionKey> {
    ...
    pub fn begin_lead_review_attempt(
        &self,
        run_id: &str,
        task_id: &str,
        _event_kind: super::completion_routing::TeamRoutingEventKind,
    ) {
        *self.active_lead_review.lock().unwrap() = Some(ActiveLeadReviewContext {
            run_id: run_id.to_string(),
            task_id: task_id.to_string(),
        });
    }

    pub fn end_lead_review_attempt(&self, run_id: &str) {
        let mut active = self.active_lead_review.lock().unwrap();
        if active.as_ref().map(|ctx| ctx.run_id.as_str()) == Some(run_id) {
            *active = None;
        }
    }
    ...
    fn routing_stats(&self) -> TeamRoutingStats {
        ...
        for record in &pending {
            if record.envelope.requester_session_key.is_none()
                && record.envelope.fallback_session_keys.is_empty()
            {
    ...
    fn flush_pending_routing_events_locked(
        ...
        for record in pending {
            if !record.ready_for_retry(now) {
                remaining.push(record);
                continue;
            }
            let pending_on_error = record
                .clone()
                .with_delivery_status(RoutingDeliveryStatus::PersistedPending);
            if let Err(err) = tx.try_send(TeamNotifyRequest {
                ...
                tracing::warn!(
                    team_id = %self.session.team_id,
                    task_id = %record.envelope.event.task_id,
    ...
    pub fn persist_pending_routing_record(&self, record: PendingRoutingRecord) {
        let _guard = self.pending_store_lock.lock().unwrap();
        if let Err(err) = self.session.upsert_pending_completion_record(&record) {
            tracing::error!(
                team_id = %self.session.team_id,
                task_id = %record.envelope.event.task_id,
                error = %err,
    ...
    pub fn retry_due_pending_routing_events(&self) {
        self.flush_pending_routing_events();
    }
...
mod tests {
    ...
    #[test]
    fn bootstrap_workspace_artifacts_writes_base_team_files_without_leaving_planning() {
        let (orch, tmp) = make_orchestrator();
        orch.set_lead_agent_name("claude-alpha".into());
        orch.set_available_specialists(vec!["codex-beta".into()]);

        orch.bootstrap_workspace_artifacts().unwrap();

        assert!(tmp.path().join("TEAM.md").is_file());
        assert!(tmp.path().join("AGENTS.md").is_file());
        assert!(tmp.path().join("CONTEXT.md").is_file());
        assert!(tmp.path().join("HEARTBEAT.md").is_file());
        assert!(tmp.path().join("TASKS.md").is_file());
    ...
    fn test_handle_specialist_missing_completion_holds_claim_and_logs_diagnostic() {
        ...
        assert!(matches!(
            task.status_parsed(),
            crate::agent_core::team::registry::TaskStatus::Held {
                ref reason,
                ref agent,
                ..
    ...
    #[tokio::test]
    async fn review_required_pending_event_stays_persisted_until_explicit_delivery_resolution() {
        let (orch, _tmp) = make_orchestrator();
        let envelope = TeamRoutingEnvelope {
            ...
            event: TeamRoutingEvent::submitted("T200", "codex", "ready for review"),
        };
        orch.session
            .append_pending_completion_record(&PendingRoutingRecord::from_envelope(
                envelope.clone(),
            ))
            .unwrap();
        ...
        let replayed = timeout(Duration::from_secs(2), rx.recv())
            ...
            .expect("pending replay timed out")
            .expect("pending replay channel closed");
        assert_eq!(replayed.envelope.run_id, envelope.run_id);

        let pending = orch.session.load_pending_completions().unwrap();
        assert_eq!(pending.len(), 1);
        assert_eq!(pending[0].run_id(), envelope.run_id);
        assert!(pending[0]
            .review
            .as_ref()
            .and_then(|review| review.next_attempt_at.as_ref())
            .is_some());
    }
    ...
    #[test]
    fn persist_pending_routing_record_upserts_by_run_id() {
        let (orch, _tmp) = make_orchestrator();
        let envelope = TeamRoutingEnvelope {
            run_id: "run-upsert".into(),
            parent_run_id: None,
            requester_session_key: Some(SessionKey::new("ws", "group:test-team")),
            fallback_session_keys: vec![],
            delivery_source: None,
        ...
        assert_eq!(
            pending[0]
                .review
                .as_ref()
                .map(|review| review.attempt_count),
            Some(1)
        );
    }

    #[test]
    fn post_message_records_active_review_task_id() {
        let (orch, _tmp) = make_orchestrator();
        orch.set_lead_agent_name("claude-alpha".into());
        orch.begin_lead_review_attempt(
            "run-review",
            "T202",
            crate::agent_core::team::completion_routing::TeamRoutingEventKind::TaskFailed,
        );
        assert!(orch.post_message("user-facing failure update"));
        orch.end_lead_review_attempt("run-review");

        let latest = orch.session.load_latest_leader_update().unwrap().unwrap();
        assert_eq!(latest.task_id.as_deref(), Some("T202"));
        assert_eq!(
            latest.kind,
    ...
    fn routing_envelope_without_live_target_stays_pending_and_reports_missing_target() {
        ...
        let pending = orch.session.load_pending_completions().unwrap();
        assert_eq!(pending.len(), 1);
        assert!(pending[0].envelope.requester_session_key.is_none());
        assert!(pending[0].envelope.fallback_session_keys.is_empty());
    ...
    fn flush_pending_routing_events_preserves_unsent_records_when_channel_is_full() {
        ...
        assert_eq!(pending[0].envelope.event.task_id, "T001");
        assert_eq!(pending[1].envelope.event.task_id, "T002");
    }
    ...
    async fn test_accept_submitted_task_triggers_all_done_milestone() {
        ...
        orch.set_scope(crate::protocol::SessionKey::new("test", "test-scope"));
    ...
    fn collect_events(
        ...
        orch.set_scope(crate::protocol::SessionKey::new("lark", "group:test"));
        events
    }
