...
use super::completion_routing::{
    PendingRoutingRecord, ReviewAttemptDiagnostic, TeamRoutingEnvelope,
};
...
impl TeamSession {
    ...
    pub fn append_pending_completion(&self, envelope: &TeamRoutingEnvelope) -> Result<()> {
        self.append_pending_completion_record(&PendingRoutingRecord::from_envelope(
            envelope.clone(),
        ))
    }

    pub fn append_pending_completion_record(&self, record: &PendingRoutingRecord) -> Result<()> {
        use std::io::Write;
    ...
    pub fn upsert_pending_completion_record(&self, record: &PendingRoutingRecord) -> Result<()> {
        let mut pending = self.load_pending_completions()?;
        let mut replaced = false;
        for existing in &mut pending {
        ...
        self.replace_pending_completions(&pending)
    }

    pub fn load_pending_completions(&self) -> Result<Vec<PendingRoutingRecord>> {
        let path = self.dir.join("pending-completions.jsonl");
        if !path.exists() {
            return Ok(vec![]);
        }
        ...
        content
            ...
            .map(|line| {
                serde_json::from_str::<PendingRoutingRecord>(line).or_else(|_| {
                    serde_json::from_str::<TeamRoutingEnvelope>(line)
                        .map(PendingRoutingRecord::from_envelope)
                })
            })
            .collect::<std::result::Result<Vec<PendingRoutingRecord>, _>>()
            .map_err(Into::into)
    }
    ...
    pub fn replace_pending_completions(&self, records: &[PendingRoutingRecord]) -> Result<()> {
        let path = self.dir.join("pending-completions.jsonl");
        if records.is_empty() {
            if path.exists() {
                std::fs::remove_file(path)?;
    ...
    pub fn record_leader_update(
        ...
        lead_delivery_source: Option<&crate::agent_core::turn_context::TurnDeliverySource>,
        source_agent: &str,
    ...
    pub fn has_post_update_for_task_since(
        ...
        for line in content.lines().rev() {
            ...
            let Ok(record) = serde_json::from_str::<LeaderUpdateRecord>(line) else {
                continue;
            };
            if record.kind != LeaderUpdateKind::PostUpdate {
                continue;
            }
    ...
    pub fn build_task_reminder(&self, task: &Task, registry: &TaskRegistry) -> String {
        ...
        let resume_note = match task.status_parsed() {
            crate::agent_core::team::registry::TaskStatus::Claimed { agent, at } => {
...
mod tests {
    ...
    fn test_pending_completion_round_trip() {
        ...
        assert_eq!(loaded[0].envelope, envelope);
        assert_eq!(
            loaded[0].review.as_ref().map(|review| review.review_kind),
            Some(crate::agent_core::team::completion_routing::ReviewRequiredKind::Failed)
        );
    ...
    #[test]
    fn test_pending_completion_round_trip_with_review_failure_metadata() {
        let (session, _tmp) = make_session();
        let record = PendingRoutingRecord::from_envelope(TeamRoutingEnvelope {
            ...
            team_id: "team-001".into(),
            delivery_status: RoutingDeliveryStatus::PersistedPending,
            event: TeamRoutingEvent::submitted("T999", "codex", "done"),
            delivery_source: None,
        })
        .note_failed_attempt(
            crate::agent_core::team::completion_routing::ReviewFailureClassification::StillRequiresResolution,
            "lead turn finished without accept/reopen",
            Some("2026-03-18T14:00:00Z".to_string()),
        );

        session.append_pending_completion_record(&record).unwrap();
        let loaded = session.load_pending_completions().unwrap();
        assert_eq!(loaded, vec![record]);
    }

    #[test]
    fn test_load_pending_completion_legacy_envelope_shape() {
        let (session, _tmp) = make_session();
        let envelope = TeamRoutingEnvelope {
        ...
        std::fs::write(
            session.dir.join("pending-completions.jsonl"),
    ...
    fn test_remove_pending_completion_by_run_id() {
        ...
        let loaded = session.load_pending_completions().unwrap();
        assert_eq!(loaded.len(), 1);
        assert_eq!(loaded[0].envelope, second);
        assert!(!session
