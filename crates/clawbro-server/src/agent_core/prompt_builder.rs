...
use crate::agent_core::memory::cap_to_words;
use crate::agent_core::traits::AgentRole;
use crate::skills_internal::PersonaSkillData;
...
只有纯检查点/一般进展通知在“不需要告知用户、也不需要调整执行”时，才可以直接结束回复。
对于需要 Lead 明确决策的通知（例如 submitted 待验收、blocked、failed、missing completion），不能静默结束。
...
**收到任务提交待验收通知时：**
- 先检查结果工件，再在结束前**必须二选一**：
  - 调用 `accept_task(task_id)` 验收，或
  - 调用 `reopen_task(task_id, reason)` 退回给 Specialist 修正
- 不能直接结束这一轮；如果没有调用上述任一工具，这一轮就是协议违规
    ...
    pub fn build(&self) -> String {
        ...
        if let Some(persona) = self.persona {
            ...
            if let Some(mbti) = &id.mbti_str {
                if let Some(mt) = crate::skills_internal::MbtiType::from_str(mbti) {
...
mod tests {
    use super::*;
    use crate::skills_internal::{IdentityData, PersonaSkillData};
