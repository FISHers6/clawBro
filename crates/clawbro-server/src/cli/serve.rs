...
pub async fn run(args: ServeArgs) -> Result<()> {
    ...
    let gateway_bin = which::which("clawbro-gateway")
        .context("clawbro-gateway not found — ensure it's installed and in PATH")?;
    ...
    #[cfg(not(unix))]
    {
        let status = std::process::Command::new(&gateway_bin)
            .status()
            .context("Failed to start clawbro-gateway")?;
        std::process::exit(status.code().unwrap_or(1));
    }
}

fn load_dot_env() {
    let path = dirs::home_dir().unwrap_or_default().join(".clawbro").join(".env");
