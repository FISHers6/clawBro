...
pub async fn run() -> Result<()> {
    ...
    if cfg_path.exists() {
        ...
        match toml::from_str::<toml::Value>(&content) {
            Ok(_)  => println!("  {} ~/.clawbro/config.toml (valid TOML)", style("✓").green()),
            Err(e) => {
        ...
        println!("  {} config.toml missing — run: clawbro setup", style("✗").red());
        issues += 1;
    }
    ...
    for var in ["ANTHROPIC_API_KEY", "OPENAI_API_KEY"] {
        match std::env::var(var) {
            ...
            _ => println!("  {} {} not set (run: source ~/.clawbro/.env)", style("–").yellow(), var),
        }
    }
    ...
    for sub in ["sessions", "shared", "skills"] {
        ...
        if p.exists() {
            ...
            println!("  {} ~/.clawbro/{} missing (mkdir -p ~/.clawbro/{})", style("✗").red(), sub, sub);
            issues += 1;
        }
    }
