...
fn ensure_role_eligibility(
    ...
    intent: &TurnIntent,
    session: &RuntimeSessionSpec,
    profile: &crate::runtime::backend::CapabilityProfile,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    async fn conductor_uses_specialist_role_for_non_leader_team_target() {
        ...
        conductor
            .execute(TurnIntent {
                session_key: crate::protocol::SessionKey::new("relay", "group:test:openclaw"),
                mode: TurnMode::Team,
    ...
    async fn conductor_allows_relay_when_backend_is_relay_only() {
        ...
        let result = conductor
            .execute(TurnIntent {
                session_key: crate::protocol::SessionKey::new("relay", "group:test:relay"),
                mode: TurnMode::Relay,
