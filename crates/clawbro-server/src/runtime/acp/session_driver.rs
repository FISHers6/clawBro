...
fn apply_provider_profile_for_acp_backend(
    ...
    match &profile.protocol {
        crate::runtime::RuntimeProviderProtocol::OfficialSession => Ok(config),
        crate::runtime::RuntimeProviderProtocol::AnthropicCompatible {
            base_url,
            auth_token,
            default_model,
            small_fast_model,
        ...
        crate::runtime::RuntimeProviderProtocol::OpenaiCompatible { .. } => {
...
fn apply_codex_projection_for_acp_backend(
    ...
    match codex_projection {
        ...
        Some(CodexProjectionMode::LocalConfig) => {
            ...
            if let crate::runtime::RuntimeProviderProtocol::OpenaiCompatible { api_key, .. } =
                &profile.protocol
            {
...
mod tests {
    ...
    fn acp_prompt_blocks_preserve_rendered_recent_history() {
        let session = RuntimeSessionSpec {
            ...
            session_key: crate::protocol::SessionKey::new("ws", "history:test"),
            role: crate::runtime::contract::RuntimeRole::Solo,
            workspace_dir: None,
            ...
            context: crate::runtime::contract::RuntimeContext {
                history_lines: vec![
                    "[user]: [alice]: 第一条".into(),
                ...
                ..crate::runtime::contract::RuntimeContext::default()
            },
        };
        ...
        let session = RuntimeSessionSpec {
            ...
            context: crate::runtime::contract::RuntimeContext {
                ...
                history_messages: vec![
                    crate::runtime::contract::RuntimeHistoryMessage {
                        role: "user".into(),
                        content: "第一条".into(),
    ...
    fn generic_acp_backends_use_generic_policy() {
        use crate::runtime::acp::policy::{AcpBackendPolicy, BootstrapStyle};
        // Qwen: explicit generic
        let qwen = AcpBackendPolicy::for_backend(Some(crate::runtime::acp::AcpBackend::Qwen));
    ...
    fn emitted_session_id_is_some_on_new_path_and_none_on_load_path() {
        ...
        let new_turn = crate::runtime::contract::TurnResult {
            full_text: "ok".into(),
            events: vec![],
        ...
        // load_session path:
        let load_turn = crate::runtime::contract::TurnResult {
            full_text: "ok".into(),
            events: vec![],
    ...
    fn resume_branch_requires_strategy_plus_capability_plus_prior_id() {
        use crate::runtime::acp::policy::ResumeStrategy;
