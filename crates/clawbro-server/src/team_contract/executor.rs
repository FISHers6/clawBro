...
use super::schema::{TeamToolCall, TeamToolResponse};
use super::visibility::ensure_team_call_allowed;
...
pub async fn execute_team_contract_call(
    team_orch: Arc<TeamOrchestrator>,
    role: RuntimeRole,
    call: TeamToolCall,
    ...
    let response = match call {
        ...
        TeamToolCall::AssignTask {
            ...
            team_orch.emit_task_updated(&task_id);
...
mod tests {
    ...
    async fn executor_registers_task_through_canonical_business_path() {
        ...
        let response = execute_team_contract_call(
            Arc::clone(&orch),
            RuntimeRole::Leader,
            TeamToolCall::CreateTask {
                id: Some("T500".into()),
