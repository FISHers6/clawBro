...
/// WeChat renders messages as plain text, so raw markdown looks bad.
fn strip_markdown(text: &str) -> String {
    ...
    static RE_BOLD_STAR: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"\*\*([^*]+)\*\*").unwrap());
    static RE_BOLD_UNDER: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"__([^_]+)__").unwrap());
    static RE_ITALIC_STAR: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\*([^*]+)\*").unwrap());
    static RE_ITALIC_UNDER: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"_([^_]+)_").unwrap());
...
/// Handles text items (type=1), voice items (type=3), and ref_msg quotes.
...
/// Derive session scope from from_user_id.
/// WeChat iLink is currently 1:1 only.
...
/// Cache of context_token per user_id. iLink requires context_token when sending replies.
...
/// Per-user typing_ticket cache obtained via getConfig endpoint.
/// The official plugin caches for up to 24h with exponential backoff retry.
struct TypingTicketCache {
    /// user_id -> (typing_ticket, expiry_timestamp_ms)
    inner: Mutex<HashMap<String, (String, u64)>>,
}

const TYPING_TICKET_TTL_MS: u64 = 24 * 60 * 60 * 1000; // 24 hours
...
pub struct WeChatChannel {
    ...
    /// Timestamp (ms since epoch) until which all API calls are paused (session expiry).
...
impl WeChatChannel {
    ...
    /// Check if session is paused due to expiry (errcode=-14).
    ...
    /// Pause all API calls for SESSION_PAUSE_DURATION_MS (1 hour).
    ...
    /// Fetch typing_ticket for a user via getConfig endpoint.
...
impl Channel for WeChatChannel {
    ...
    async fn send(&self, msg: &OutboundMsg) -> Result<()> {
        ...
        // Official protocol: body wraps message in "msg" envelope + "base_info"
        let body = serde_json::json!({
            "msg": {
                "from_user_id": "",
    ...
    async fn listen(&self, tx: mpsc::Sender<InboundMsg>) -> Result<()> {
        let checker = crate::channels_internal::AllowlistChecker::load();
        ...
        while !self.cancel.is_cancelled() {
            ...
            // Include base_info in getupdates request (matches official protocol)
            let body = serde_json::json!({
            ...
            let resp = match result {
                ...
                Err(e) => {
                    ...
                    if consecutive_failures >= MAX_CONSECUTIVE_FAILURES {
                        tracing::warn!("WeChat: {MAX_CONSECUTIVE_FAILURES} consecutive failures, backing off {BACKOFF_DELAY_MS}ms");
            ...
            if let Some(t) = updates.longpolling_timeout_ms {
                if t > 0 {
                    next_timeout_ms = t;
                    tracing::debug!("WeChat: updated poll timeout to {t}ms");
                }
            }

            // Application-level error check (ret/errcode)
    ...
    async fn update_typing(&self, scope: &str) -> Result<()> {
        ...
        let context_token = match self.context_tokens.get(to_user_id) {
            Some(t) if !t.is_empty() => t,
            _ => return Ok(()), // No context_token, can't send typing
        };

        // Fetch typing_ticket via getConfig (cached for 24h)
        ...
        // Official protocol: sendTyping uses ilink_user_id + typing_ticket + status
        let body = serde_json::json!({
...
mod tests {
    ...
    fn test_send_body_structure() {
        ...
        // Verify base_info exists at top level
    ...
    fn test_getupdates_response_parsing() {
        ...
        // Only first message is user message (type=1)
    ...
    fn test_getupdates_session_expired() {
        ...
        assert_eq!(resp.ret, Some(SESSION_EXPIRED_ERRCODE));
    }
    ...
    fn test_generate_client_id() {
        ...
        assert!(id.len() > "clawbro-wechat:".len());
    ...
    fn test_sendtyping_body_structure() {
        let user_id = "user1@im.wechat";
        let typing_ticket = "ticket_abc";
        ...
        assert_eq!(body["ilink_user_id"], user_id);
        assert_eq!(body["typing_ticket"], typing_ticket);
        assert_eq!(body["status"], 1);
        assert!(body["base_info"]["channel_version"].is_string());
    }
    ...
    fn test_strip_markdown() {
        ...
        // Mixed
        assert_eq!(
            strip_markdown("**Hello** `world` [link](url)"),
            "Hello world link"
        );
    }

    #[test]
    fn test_getupdates_request_body_has_base_info() {
        let sync_buf = "some_cursor";
        let body = serde_json::json!({
            "get_updates_buf": sync_buf,
            "base_info": { "channel_version": CHANNEL_VERSION }
        });
        assert_eq!(body["get_updates_buf"], "some_cursor");
        assert_eq!(body["base_info"]["channel_version"], CHANNEL_VERSION);
    }
