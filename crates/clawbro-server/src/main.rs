...
use clawbro_protocol::parse_session_key_text;
use clawbro_runtime::{
    acp::AcpBackendAdapter, ApprovalBroker, BackendRegistry, OpenClawBackendAdapter,
    QuickAiNativeBackendAdapter,
};
use clawbro_server::channel_registry::ChannelRegistry;
use clawbro_server::config;
use clawbro_server::delivery_resolver::resolve_delivery;
use clawbro_server::gateway;
use clawbro_server::im_sink::spawn_im_turn;
use clawbro_server::state::{AppState, BrokerApprovalResolver};
...
async fn main() -> Result<()> {
    ...
    use clawbro_agent::memory::triggers::default_triggers;
    use clawbro_agent::memory::{AcpDistiller, FileMemoryStore, MemorySystem};
    ...
    if let Some(dt_cfg) = &cfg.channels.dingtalk {
        if dt_cfg.enabled {
            ...
            if let Ok(dt_config) = clawbro_channels::dingtalk::DingTalkConfig::from_env() {
                let channel = Arc::new(clawbro_channels::DingTalkChannel::new(
                    dt_config,
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        match clawbro_channels::Channel::listen(channel.as_ref(), tx.clone()).await {
                            Err(e) => {
                ...
                tokio::spawn(async move {
                    while let Some(inbound) = rx.recv().await {
                        spawn_im_turn(
                            ...
                            channel_clone.clone() as Arc<dyn clawbro_channels::Channel>,
            ...
            match lark_instances {
                Ok(instances) => {
                    ...
                    for (index, instance) in instances.into_iter().enumerate() {
                        ...
                        let channel = Arc::new(clawbro_channels::LarkChannel::new_with_instance(
                            instance.id.clone(),
                    ...
                    for (instance_id, channel, is_default) in listeners {
                        ...
                        tokio::spawn(async move {
                            let mut delay = Duration::from_secs(5);
                            loop {
                                match clawbro_channels::Channel::listen(channel.as_ref(), tx.clone())
                                    .await
                                {
                                    Err(e) => {
                                        delay = next_delay(delay);
    ...
    let cron_store = Arc::new(clawbro_cron::CronStore::open(&cron_db)?);
    ...
    // session so operators can resolve them with `/approve <id> <decision>`.
    {
        ...
        tokio::spawn(async move {
            while let Ok(event) = approval_rx.recv().await {
                let clawbro_protocol::AgentEvent::ApprovalRequest {
                    session_key,
                    approval_id,
                    prompt,
                    command,
    ...
    // does not recurse into the relay result.
    {
        ...
        let relay_dispatch: clawbro_agent::relay::RelayDispatchFn =
            Box::new(move |target_agent, content, lead_session_key| {
                ...
                Box::pin(async move {
                    ...
                    let relay_session_key = clawbro_protocol::SessionKey::new("relay", &relay_scope);
                    ...
                    let msg = clawbro_protocol::InboundMsg {
                        ...
                        source: clawbro_protocol::MsgSource::Relay,
                    };
                    registry
        ...
        registry.set_relay_engine(std::sync::Arc::new(clawbro_agent::relay::RelayEngine::new(
            relay_dispatch,
        )));
    ...
    let (redispatch_tx, mut redispatch_rx) =
        tokio::sync::mpsc::channel::<clawbro_protocol::InboundMsg>(256);
    {
        let bot_names: Vec<String> = cfg.agent_roster.iter().map(|e| e.name.clone()).collect();
        if !bot_names.is_empty() {
            let trigger = std::sync::Arc::new(clawbro_channels::mention_trigger::MentionTrigger::new(
                bot_names,
                redispatch_tx.clone(),
            ));
    ...
    clawbro_server::team_runtime::wire_team_runtime(
        registry.clone(),
        &cfg,
    ...
    // 启动 CronScheduler
    {
        ...
        let cron_trigger: clawbro_cron::TriggerFn = Arc::new(
            move |session_key_str: String,
                ...
                tokio::spawn(async move {
                    ...
                    if let Some(ref cond_str) = condition {
                        if let Some(cond) = clawbro_cron::CronCondition::parse(cond_str) {
                            match cond {
                                clawbro_cron::CronCondition::IdleGtSeconds(threshold) => {
                                    let idle = registry
                    ...
                    let session_key = if session_key_str.contains(':') {
                        parse_session_key_text(&session_key_str).unwrap_or_else(|_| {
                            clawbro_protocol::SessionKey::new("cron", session_key_str.as_str())
                        })
                    } else {
                        clawbro_protocol::SessionKey::new("cron", session_key_str.as_str())
                    };
                    let msg = clawbro_protocol::InboundMsg {
                        ...
                        content: clawbro_protocol::MsgContent::text(prompt),
                        sender: "cron".to_string(),
                        channel: "cron".to_string(),
                        timestamp: chrono::Utc::now(),
                        thread_ts: None,
                        target_agent: agent_opt,
                        source: clawbro_protocol::MsgSource::Cron,
                    };
                    match registry
                        .handle_with_context(msg, TurnExecutionContext::default())
                        .await
                    {
                        Ok(Some(result)) => {
                            ...
                            if let Some(ms) = &memory {
                                ...
                                ms.emit(clawbro_agent::MemoryEvent::CronJobCompleted {
                                    scope: session_key,
        ...
        let scheduler = clawbro_cron::CronScheduler::new(cron_store, cron_trigger);
            ...
            loop {
                ...
                if !agent_dirs.is_empty() {
                    for scope in registry_clone.all_active_scopes() {
                        ms_clone.emit(clawbro_agent::MemoryEvent::NightlyConsolidation {
                            scope,
